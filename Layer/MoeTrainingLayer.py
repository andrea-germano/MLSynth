...
class MoeTrainingLayer(BaseTrainingLayer):
    """An implementation of a Transformer Mixture of Experts Layer.
       ...
       ⚠ INCOMPLETE — kept as the ORIGINAL implementation on purpose, do not trust its output:
       - NOT aligned with the unified DenseBlockMath cost model (inline legacy formulas:
         no GQA/SwiGLU, FLOPs not divided by tp, single shared tensor_size);
       - graph wiring is partial: the TP allreduces and the backward attention/ffwd nodes
         have no parents, so the emitted graph is largely disconnected;
       TODO: Implement routing
       TODO: Implement imbalanced all-to-all collectives
       TODO: align cost model with DenseBlockMath and wire the dependency chain
    """
