...
class InferenceLayer(ABC):
    ...
    * decode: the phase where the model is generating new tokens based on the filled context window, attending to the exixsting KV cache
    ...
    @abstractmethod
    def prefill(self, name: str = "node_prefill", pg_name: str | None = None, num_batches: int = 1, prompt_len: int = 1) -> List[ChakraNode]:
        """Return Chakra nodes for the prefill phase of this layer."""
        raise NotImplementedError
    
    @abstractmethod
    def decode(self, name: str = "node_decode", pg_name: str | None = None, num_batches: int = 1, kv_len: int = 1) -> List[ChakraNode]:
